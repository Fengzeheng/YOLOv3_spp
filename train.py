...
def train(hyp):
    ...
    accumulate = max(round(64 / batch_size), 1)  # bs=4, accumulate=16
    #  accumulate n times before optimizer update (bs 64)，每64/batch_size个batch更新一次参数，而不是每个batch更新
    weights = opt.weights  # initial training weights
    imgsz_train = opt.img_size
    imgsz_test = opt.img_size  # test image sizes为512
    multi_scale = opt.multi_scale

    # Image sizes
    # 图像要设置成32的倍数
    gs = 32  # (pixels) grid size
    assert math.fmod(imgsz_test, gs) == 0, "--img-size %g must be a %g-multiple" % (imgsz_test, gs)  # 判断输入图像大小是否为32整数倍
    grid_min, grid_max = imgsz_test // gs, imgsz_test // gs
    if multi_scale:
        imgsz_min = opt.img_size // 1.5  # 给出多尺度训练的范围
        imgsz_max = opt.img_size // 0.667

        # 将给定的最大，最小输入尺寸向下调整到32的整数倍
        grid_min, grid_max = imgsz_min // gs, imgsz_max // gs  # //向下取整，数据类型与被除数一致
        imgsz_min, imgsz_max = int(grid_min * gs), int(grid_max * gs)  # 整形变换
        imgsz_train = imgsz_max  # initialize with max size： int(512//0.667//gs*gs)=736
        print("Using multi_scale training, image range[{}, {}]".format(imgsz_min, imgsz_max))

    # configure run
    # init_seeds()  # 初始化随机种子，保证结果可复现
    data_dict = parse_data_cfg(data)  # 解析my_data.data文件
    ...
    for f in glob.glob(results_file):  # 返回指定目录下的所有文件，这里就遍历返回一.txt文件下的内容
        os.remove(f)  # 由于results_file是训练时实时创建的，故这里没有需要清除的内容
    ...
    if opt.freeze_layers:
        ...
        # 若要训练全部权重，删除以下代码
        darknet_end_layer = 74  # only yolov3-spp cfg
    ...
    # 使用amp:混合精度训练，此处实例化一个GradScaler对象，用来损失放大避免激活梯度值太小造成溢出从而无法收敛
    scaler = torch.cuda.amp.GradScaler() if opt.amp else None

    start_epoch = 0
    best_map = 0.0
    if weights.endswith(".pt") or weights.endswith(".pth"):  # 判断是否有预训练权重文件
        ckpt = torch.load(weights, map_location=device)  # 载入权重文件到设备内存

        # load model
        try:
            ckpt["model"] = {k: v for k, v in ckpt["model"].items() if model.state_dict()[k].numel() == v.numel()}  # .numel()返回数组中元素的个数
        ...
        # load results
        if ckpt.get("training_results") is not None:  # 输出上一次中断训练的最后训练结果
            with open(results_file, "w") as file:
                file.write(ckpt["training_results"])  # write results.txt

        # epochs = opt.epochs
        # 注：官方的预训练权重默认epoch=-1，且不保存training_results
        start_epoch = ckpt["epoch"] + 1  # 开始轮次为上次训练完成的轮次+1，并加载上次最后轮次训练得到的权重，即可接着训练并连续展示
    ...
    lf = lambda x: ((1 + math.cos(x * math.pi / epochs)) / 2) * (1 - hyp["lrf"]) + hyp["lrf"]  # 定义一个cosine学习率
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset,
                                                   ...
                                                   pin_memory=True,  # pin_memory=True表示将load进的数据拷贝进锁页内存区，将内存中的Tensor转移至GPU cuda区会很快
                                                   collate_fn=train_dataset.collate_fn)  #  collate_fn：如何取样本的，我们可以定义自己的函数来准确地实现想要的功能
    ...
    # coco = None
    coco = get_coco_api_from_dataset(val_dataset)  # 事先遍历一遍验证集，读取一遍标签信息

    print("starting traning for %g epochs..." % epochs)
    print('Using %g dataloader workers' % nw)

    # 开始迭代训练
    for epoch in range(start_epoch, epochs):  # mloss：平均损失
        ...
        if opt.notest is False or epoch == epochs - 1:  # notest默认为False，每个epoch验证一次
            ...
            with open(results_file, "a") as f:
                # 记录coco的12个指标加上训练总损失和lr在results.txt中
            ...
            if opt.savebest is False:  # 每一个epoch训练完保存权重文件
                # save weights every epoch
                with open(results_file, 'r') as f:  # 权重文件实际是一字典形式,其通常与results.txt文件互转
                    save_files = {
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()  # 实例化一个parser
    ...
    parser.add_argument('--multi-scale', type=bool, default=True,
                        help='adjust (67%% - 150%%) img_size every 10 batches')  # 按照67%-150%随机缩放512
    parser.add_argument('--img-size', type=int, default=512, help='test size')  # 给定测试时候图片输入的大小
    parser.add_argument('--rect', action='store_true', help='rectangular training')  # action参数：有--rect出现则将rect的值设为True
    parser.add_argument('--savebest', type=bool, default=False, help='only save best checkpoint')  # 每次验证后都保存一次权重，方便某次停止后接着继续训练
    parser.add_argument('--notest', action='store_true', help='only test final epoch')  # 每个epoch都验证一次，默认为False
    ...
    parser.add_argument('--weights', type=str, default='weights/yolov3-spp-ultralytics-512.pt',
                        help='initial weights path')  # 存放与训练权重或设置为上一次停止训练后的权重接着上一次训练
    ...
    with open(opt.hyp) as f:
        hyp = yaml.load(f, Loader=yaml.FullLoader)  # 载入超参数文件，将yaml转为字典
