...
def parse_xml_to_dict(xml):  # 解析xml文件成一嵌套字典
    """
    ...
    """
    # VOC2008开始一般就<source>、<size>、<object>的len不为0
    if len(xml) == 0:  # 遍历到底层，直接返回tag对应的信息， 如：<folder>VOC2012</folder>的len为0
        return {xml.tag: xml.text}

    result = {}
    for child in xml:  # 遍历<class 'lxml.etree._Element'>中的Element
        child_result = parse_xml_to_dict(child)  # 递归遍历标签信息
        if child.tag != 'object':
            result[child.tag] = child_result[child.tag]
        else:
            if child.tag not in result:  # 因为object可能有多个，所以需要放入列表里, 如{'object': []}
                result[child.tag] = []
...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='train'):
    ...
    :return:
    """
    save_txt_path = os.path.join(save_root, train_val, "labels")  # 该路径存储每个训练数据对应的标注信息文件.txt
    if os.path.exists(save_txt_path) is False:
        os.makedirs(save_txt_path)
    save_images_path = os.path.join(save_root, train_val, "images")  # 该路径存储每个训练数据图片.jpg
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        ...
        with open(xml_path) as fid:
            xml_str = fid.read()
        xml = etree.fromstring(xml_str)  # xml的type为<class 'lxml.etree._Element'>
        ...
        with open(os.path.join(save_txt_path, file + ".txt"), "w") as f:
            for index, obj in enumerate(data["object"]):
                ...
                class_name = obj["name"]
                class_index = class_dict[class_name] - 1  # 目标id从0开始，因为Pascal voc是从1开始
                ...
                if index == 0:
                    f.write(" ".join(info))  # join() 方法用于将序列中的元素以指定的字符连接生成一个新的字符串
                else:
                    f.write("\n" + " ".join(info))
        ...
        if os.path.exists(path_copy_to) is False:
            shutil.copyfile(img_path, path_copy_to)  # shutil.copyfile将源文件的内容复制到目标文件中
...
def main():
    ...
    with open(train_txt_path, "r") as r:
        train_file_names = [i for i in r.read().splitlines() if len(i.strip()) > 0]  # train_file_names=[2008_000008,2008_000015,...]
