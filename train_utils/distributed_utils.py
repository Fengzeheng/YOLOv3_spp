...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        iter_time = SmoothedValue(fmt='{avg:.4f}')  # 实现了__str__()
        data_time = SmoothedValue(fmt='{avg:.4f}')  # 实现了__str__()
        ...
        MB = 1024.0 * 1024.0
        for obj in iterable:
            data_time.update(time.time() - end)  # 更新每次迭代到数据的时间
            yield obj  # 返回iterable中的元素
            iter_time.update(time.time() - end)  # 更新迭代处理时间
...
def warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor):
    ...
    return torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=f)  # lr_lambda传入一个函数或一个以函数为元素列表，作为学习率调整的策略,默认参数是当前epoch
