...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
                    ...
                    multi_scale=False, warmup=False, scaler=None):  # accumulate默认输入为16
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")  # metric_logger对象用来更新显示各项指标结果
    ...
    if epoch == 0 and warmup is True:  # 当训练第一轮（epoch=0）时，启用warmup训练方式，可理解为热身训练
        ...
        warmup_iters = min(1000, len(data_loader) - 1)  # pascalvoc训练集5717张，bs=4，所以这里warmup_iters=1000

        lr_scheduler = utils.warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor)  # 这里给epoch==0训练时的学习率设为0.001倍
        accumulate = 1
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):  # 处理每个batch数据
        ...
        if multi_scale:
            ...
            if ni % accumulate == 0:  # adjust img_size (67% - 150%) every 1 batch，除epoch=0以外，每个epoch中经过16个batch就调整size大小
                # 在给定最大最小输入尺寸范围内随机选取一个size(size为32的整数倍)
                img_size = random.randrange(grid_min, grid_max + 1) * gs  # img_size在[320,736]中选取
            sf = img_size / max(imgs.shape[2:])  # scale factor imgs.shape=(4, 3, 736,736)

            # 如果图片最大边长不等于img_size, 则缩放图片，并将长和宽调整到32的整数倍
            if sf != 1:
                # gs: (pixels) grid size
                ns = [math.ceil(x * sf / gs) * gs for x in imgs.shape[2:]]  # new shape (stretched to 32-multiple)
                imgs = F.interpolate(imgs, size=ns, mode='bilinear', align_corners=False)  # 对tensor的一种插值方法
        ...
        with amp.autocast(enabled=scaler is not None):
            ...
            # loss
            loss_dict = compute_loss(pred, targets, model)  # 计算每个batch的三个损失
            losses = sum(loss for loss in loss_dict.values())

        # reduce losses over all GPUs for logging purpose  TODO
        ...
        mloss = (mloss * i + loss_items) / (i + 1)  # update mean losses mloss.shape=(4,)
        ...
        losses *= 1. / accumulate  # scale loss，因为后面是每accumulate个batch更新一次权重，这里对每个batch计算的损失进行缩放

        # backward
        if scaler is not None:
            scaler.scale(losses).backward()  # 将梯度放大，防止梯度消失
        else:
            losses.backward()
        ...
        if ni % accumulate == 0:
            if scaler is not None:
                scaler.step(optimizer)  # 把梯度的值unscale回来，判断该梯度是否为NANS或infs，不是则调用optimizer.step()更新，否则保持不变
                scaler.update()  # 判断是否增大scaler
            else:
                optimizer.step()
            optimizer.zero_grad()
    ...
    return mloss, now_lr  # mloss用于后续指标计算
