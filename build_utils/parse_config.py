# 解析yolov3-spp.cfg(模型配置)文件和数据配置文件的脚本
import os
import numpy as np
...
def parse_model_cfg(path: str):
    ...
    mdefs = []  # module definitions，为每一个模块即.cfg中的[]建立一字典放进mdefs列表中
    for line in lines:
        if line.startswith("["):  # this marks the start of a new block
            ...
            if mdefs[-1]["type"] == "convolutional":
                mdefs[-1]["batch_normalize"] = 0  # 初始化默认不适用bn，后面model.py中针对除了yolo层的前一个卷积层其它重新设为bn=1
        else:
            key, val = line.split("=")
            key = key.strip()
            val = val.strip()

            if key == "anchors":  # 见yolov3-spp.cfg文件815行
                ...
                if val.isnumeric():  # return int or float 如果是数值的情况
                    mdefs[-1][key] = int(val) if (int(val) - float(val)) == 0 else float(val)  # 这里指小数位不为0的浮点型
                else:
